...
export function PaperAttemptPage() {
  ...
  const handleExit = useCallback(async () => {
    ...
    router.push(`/pyq/${paper.slug}`)
  }, [paper, attemptId, answers, marked, currentIndex, remainingSeconds, router])
  ...
  if (!slug || !isAuthenticated) return <Redirect to="/" replace />
  if (loading) return <HaloLoader label="Loading paper" />
  if (error || !paper) return <Redirect to={`/pyq/${slug}`} replace />
      ...
      return (
        <div className="pa-intro-page">
          <div className="pa-intro-card pa-resume-card">
            ...
            <Link className="pa-intro-back-link" href={`/pyq/${paper.slug}`}>
              ← Exit to paper
            </Link>
          </div>
        </div>
    ...
    return (
      <div className="pa-result-page">
        ...
        <footer className="pa-result-footer">
          ...
          <button type="button" className="pa-btn-secondary" onClick={() => router.push(`/pyq/${paper.slug}`)}>
            Back to Paper
          </button>
          <button type="button" className="pa-btn-secondary" onClick={() => router.push(`/exam/${paper.examSlug}`)}>
            Back to Exam
          </button>
        </footer>
      </div>
              ...
              <div className="pa-q-header">
                ...
                {rq?.subject && (
                  <Link
                    ...
                    href={`/exam/${paper.examSlug}?tab=subjects&subject=${encodeURIComponent(rq.subject)}`}
                  >
                    {rq.subject}
                  </Link>
                )}
