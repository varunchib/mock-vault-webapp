    ...
    <div className="db-page">
          ...
          <div className="db-ip-list">
            {activeAttempts.map((attempt) => (
              <div className="db-ip-card" key={attempt.paperSlug}>
                ...
                <Link className="db-ip-btn" href={`/paper-attempt/${attempt.paperSlug}`}>
      ...
      <section className="db-section">
        <div className="db-section-head">
          ...
          <Link className="db-section-link" href="/exams">Browse all →</Link>
        </div>
        ...
        {enrolledExams.length === 0 ? (
          <div className="db-enroll-cta">
            ...
            <Link className="db-enroll-btn" href="/exams">Browse Exams</Link>
          </div>
        ) : (
          <div className="db-enrolled-row">
            {enrolledExams.map((exam) => (
              <Link className="db-enrolled-chip" href={`/exam/${exam.slug}`} key={exam.slug}>
            ...
            <Link className="db-enrolled-chip db-add-chip" href="/exams">
        ...
        <section className="db-section">
          ...
          <div className="db-viewed-row">
            {recentlyViewed.map((rec) => (
              <Link className="db-viewed-chip" href={`/exam/${rec.slug}`} key={rec.slug}>
                <span>{rec.icon}</span>
      ...
      <section className="db-section">
        <div className="db-section-head">
          ...
          <Link className="db-section-link" href="/exams">All tests →</Link>
        </div>

        {recentAttempts.length === 0 ? (
          <div className="db-attempts-empty">
            <p>No attempts yet — start a mock test or open a PYQ paper.</p>
            <Link className="db-enroll-btn" href="/exams">Browse Tests</Link>
          </div>
        ) : (
          <div className="db-attempt-list">
            {[...new Map(recentAttempts.map((a) => [`${a.type}-${a.slug}`, a])).values()].map((attempt) => {
              ...
              return (
                <Link className="db-attempt-row" href={href} key={`${attempt.type}-${attempt.slug}`}>
