  ...
  return (
    <div className="mr-screen">
      ...
      <div className="mr-actions">
        ...
        <Link href="/analytics" className="mr-btn primary">
...
export function MockAttemptPage() {
  ...
  if (!slug) return <Redirect to="/mock-test" replace />
  if (loading) return <HaloLoader label="Loading mock" />
  if (error || !mock) return <Redirect to="/mock-test" replace />
  ...
  if (submitted) {
    return (
      <ResultScreen
        ...
        onDashboard={() => router.push(homePathForUser(user))}
        onBack={() => router.push(`/mock-test/${mock.examSlug}`)}
      />
    )
  }
    ...
    <section className="mock-attempt-page">
      ...
      <main className="mock-attempt-grid">
        ...
        <aside className="mock-palette-panel">
          ...
          <Link href={`/mock-test/${mock.examSlug}`}>Exit mock</Link>
        </aside>
      </main>
    </section>
  )
