  ...
  const content = (
    <div className="pyq-page">
      <nav className="ep-breadcrumb" aria-label="Breadcrumb">
        <Link href={homeHref}>{isAuthenticated ? 'Dashboard' : 'Home'}</Link>
        <ChevronRight size={13} />
        <Link href={`/exam/${paper.examSlug}`}>{paper.examName}</Link>
        <ChevronRight size={13} />
      ...
      <header className="pyq-paper-header">
        ...
        <div className="pyq-paper-actions">
          ...
          {firstQuestion && (
            <button
              ...
              onClick={() => {
                ...
                router.push(`/paper-attempt/${paper.slug}`)
              }}
            >
          ...
          {paper && paperGuideMap[paper.slug] && (
            <Link
              className="pyq-action-btn"
              href={`/guide/${paperGuideMap[paper.slug]}`}
            >
            ...
            <article className={`pyq-question-card${isDeleted ? ' deleted' : ''}`} key={q.slug}>
              <div className="pyq-q-header">
                ...
                <Link
                  ...
                  href={`/exam/${paper.examSlug}?tab=subjects&subject=${encodeURIComponent(q.subject)}`}
